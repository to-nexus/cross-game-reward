# Cross GameReward Protocol - 프로젝트 요약

## 🎯 프로젝트 개요

Cross GameReward Protocol은 Native CROSS와 ERC-20 토큰을 동시에 지원하는 다중 풀 디파짓 시스템입니다. Factory + Router 구조를 통해 새로운 풀을 빠르게 배포하고, 보상 토큰을 유연하게 구성할 수 있습니다.
...
- ✅ **간소화된 권한 관리**: Owner/RewardRoot 기반 접근 제어
- ✅ **3단계 풀 관리**: Active/Inactive/Paused로 세밀한 제어
- ✅ **공정한 보상**: Zero-deposit 보호 및 제거된 토큰 정산
...
사용자                (Native CROSS / ERC-20)
   │
   ▼
CrossGameRewardRouter ──► WCROSS (래핑)
   │
   ▼
CrossGameReward (UUPS) ──► CrossGameRewardPool × N (UUPS)
```

| 컴포넌트            | 역할                                                           |
|---------------------|----------------------------------------------------------------|
| CrossGameReward        | 풀 생성/관리, 보상 토큰 추가, 풀 상태 설정, Router 지정       |
| CrossGameRewardPool    | 개별 스테이킹 풀, deposit/withdraw/claim, 3-state 관리           |
| CrossGameRewardRouter  | Native/ERC-20 스테이킹 인터페이스, WCROSS 자동 처리           |
...
2. `depositNative` 호출 → Router가 WCROSS로 래핑 후 풀에 `depositFor`
3. 출금 시 Router가 풀에서 `withdrawFor` 실행 후 WCROSS를 언래핑

### 2. ERC-20 디파짓
1. 사용자가 디파짓 토큰을 Router에 허용
2. Router가 토큰을 Pool로 전송 후 `depositFor`
3. `withdrawERC20` 시 Router가 보상은 사용자에게, 원금은 다시 사용자에게 전송

### 3. 보상 분배 및 조회
- 누구나 Pool 주소로 보상 토큰을 전송할 수 있음
- 다음 디파짓/출금/클레임 시 `_syncReward`가 자동 감지
...
- `getUserDepositInfo(poolId, user)`: 디파짓 정보 통합 조회 → `(uint depositedAmount, address[] tokens, uint[] rewards)`

### 4. Zero-Deposit 보호
- `totalDeposited=0` 상태에서 예치된 보상은 `withdrawableAmount`로 분류
- 첫 디파짓터가 이러한 보상을 받지 못하도록 보호
...
### 1. 3-State 풀 관리
- **Active (0)**: 모든 작업 허용 (deposit, withdraw, claim)
- **Inactive (1)**: withdraw와 claim만 허용
- **Paused (2)**: 모든 작업 중지

설정: `CrossGameReward.setPoolStatus(poolId, status)`

### 2. 보상 메커니즘
- **O(1) 가스**: `rewardPerToken` 누적 방식
- **Zero-deposit 보호**: 첫 디파짓터에게 불공정한 보상 방지
...
- `onlyRewardRoot()`: CrossGameReward 컨트랙트, 모든 관리 기능
- `onlyRouter`: Router 전용, `depositFor/withdrawFor`
...
3. **간소화된 권한 관리** – Owner/RewardRoot modifier 기반
...
```bash
forge test                                  # 전체 테스트
forge test --match-contract CrossGameReward   # 특정 컨트랙트
...
**Coverage:** ~100%, 주요 시나리오(멀티 풀, 제거 토큰, 라우터, Zero-deposit, 엣지 케이스) 포함
...
Cross GameReward Protocol은 **완전히 테스트되고 문서화된 프로덕션 준비 상태**입니다:
- 212개 테스트 100% 통과
- 개선된 API로 더 나은 UX
- 최적화된 이벤트 로깅
- 최신 문서 완비
