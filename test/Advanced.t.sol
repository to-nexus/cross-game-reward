...
contract AdvancedTest is BaseTest {
    ...
    function test_PoolRestartAfterEnd() public {
        ...
        // 2. poolEndTime 설정하여 시즌 2 종료 예정
        uint endTime = block.timestamp + 50;
        vm.prank(address(protocol));
        stakingPool.setPoolEndBlock(endTime);
        ...
        // 5. 풀 재시작 설정 (poolEndTime을 해제하거나 연장)
        uint restartTime = block.timestamp + 100;
        vm.startPrank(address(protocol));
        stakingPool.setNextSeasonStart(restartTime);
        // poolEndTime을 재시작 이후로 연장하여 재시작 허용
        stakingPool.setPoolEndBlock(restartTime + SEASON_DURATION * 2);
        vm.stopPrank();
    ...
    function test_VirtualToRealSeasonTransition() public {
        ...
        vm.prank(owner);
        (, address poolAddr,) = protocol.createProject("FutureProject", SEASON_DURATION, futureStartBlock, 0, owner, 0);
        ...
        (uint season, uint startTime,,) = newPool.getCurrentSeasonInfo();
        assertEq(season, 1); // Virtual season 1
        assertEq(startTime, futureStartBlock);
    ...
    function test_AdminEmergencyFunctions() public {
        ...
        uint emergencyEndBlock = block.timestamp + 20;
        vm.prank(address(protocol));
        stakingPool.setPoolEndBlock(emergencyEndBlock);

        assertEq(stakingPool.poolEndTime(), emergencyEndBlock);

        // 2. 종료 블록 도달 + 1
        vm.warp(emergencyEndBlock + 1);
    ...
    /**
     * @notice poolEndTime과 season 경계 테스트
     */
    function test_PoolEndBlockAtSeasonBoundary() public {
        stakeFor(user1, 10 ether);

        // poolEndTime을 현재 시즌 종료 전으로 설정
        (,, uint endTime,) = stakingPool.getCurrentSeasonInfo();

        // 시즌 종료 10블록 전으로 설정
        uint earlyEndBlock = endTime - 10;
        require(earlyEndBlock > block.timestamp, "Test setup error");

        vm.prank(address(protocol));
        stakingPool.setPoolEndBlock(earlyEndBlock);

        // earlyEndBlock 도달 + 1
        vm.warp(earlyEndBlock + 1);
    ...
    /**
     * @notice 가상 시즌 - poolEndTime이 시즌 중간에 오는 경우
     */
    function test_VirtualSeason_EarlyPoolEnd() public {
        // 새 프로젝트 생성 (정상적으로 시작)
        vm.startPrank(owner);
        uint futureStart = block.timestamp + 100;

        (, address newPoolAddr,) = protocol.createProject("EarlyEnd", SEASON_DURATION, futureStart, 0, owner, 0);

        StakingPool newPool = StakingPool(newPoolAddr);
        vm.stopPrank();

        // 시작 블록 이후 poolEndTime을 시즌 중간으로 설정
        vm.warp(futureStart);
        assertTrue(newPool.isSeasonActive());

        // poolEndTime을 현재부터 50블록 후로 설정 (시즌 길이보다 짧음)
        uint earlyEnd = block.timestamp + 50;
        vm.prank(address(protocol));
        newPool.setPoolEndBlock(earlyEnd);

        // earlyEnd + 1 도달 (poolEndTime은 해당 블록까지 활성)
        vm.warp(earlyEnd + 1);
        assertFalse(newPool.isSeasonActive());
    ...
    /**
     * @notice 가상 시즌 - nextSeasonStart가 poolEndTime 이후인 경우
     */
    function test_VirtualSeason_NextSeasonAfterPoolEnd() public {
        ...
        // poolEndTime 설정
        uint poolEnd = block.timestamp + 50;
        vm.prank(address(protocol));
    ...
    /**
     * @notice 가상 시즌 - poolEndTime이 정확히 다음 시즌 시작 블록인 경우
     * @dev poolEndTime이 nextSeasonStart와 같거나 작으면 가상 시즌은 시작할 수 없음
     */
    function test_VirtualSeason_PoolEndAtNextSeasonStart() public {
        ...
        // 시즌 2 종료 이후로 이동
        vm.warp(season2End + 1);
        assertFalse(stakingPool.isSeasonActive());

        // nextSeasonStart - 1 블록: 시즌 2 종료됨, 가상 시즌 미시작
        vm.warp(nextSeasonStart - 1);
        assertFalse(stakingPool.isSeasonActive());

        // nextSeasonStart 블록: poolEndTime = nextSeasonStart이므로 가상 시즌 시작 불가
        vm.warp(nextSeasonStart);
        assertFalse(stakingPool.isSeasonActive());
