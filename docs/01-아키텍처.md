...
**Code 컨트랙트 패턴 + CREATE2:**
```
StakingProtocol
    ↓
stakingPoolCodeContract.code()  // creation code 조회
    ↓
salt = keccak256(projectName, projectID, "StakingPool")  // 예측 가능한 salt 생성
    ↓
create2(code + constructorArgs, salt)  // CREATE2로 배포
    ↓
StakingPool 인스턴스 생성 (주소 예측 가능)
```
...
- 업그레이드 용이성
- **주소 예측 가능성** (CREATE2)
  - 프로젝트 이름과 ID로 주소 미리 계산
  - 크로스체인 배포 시 동일 주소 보장 가능
  - 프론트엔드에서 배포 전 주소 표시 가능
...
**주소 계산 함수:**
```solidity
computeStakingPoolAddress(projectName, projectID, seasonBlocks, ...)
computeRewardPoolAddress(projectName, projectID, stakingPool)
```
