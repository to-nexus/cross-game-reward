...
(uint balance, uint points, uint lastUpdateBlock) = 
    stakingViewer.getStakeInfo(projectID, user);
...
// 예상 보상
uint expectedReward = stakingViewer.getExpectedSeasonReward(
    projectID,
    season,
    user,
    rewardTokenAddress
...
(
    ...
    uint[] memory points,
    uint[] memory seasons
) = stakingViewer.getUserStakingSummary(user, projectIDs);
...
(
    ...
    bool claimed,
    bool finalized
) = stakingViewer.getUserSeasonData(projectID, user, 1);
...
for (uint i = 0; i < rewardTokens.length; i++) {
    uint reward = stakingViewer.getClaimableReward(
        projectID,
        user,
        1,
        rewardTokens[i]
...
) = stakingViewer.getSeasonHistory(projectID, 1, 10);
...
console.log("Pending seasons:", pending);  // 예: 24

// 2. 필요한 경우 수동 롤오버
if (pending > 50) {
    // 50개씩 나눠서 처리
    while (pending > 0) {
        uint toProcess = pending > 50 ? 50 : pending;
        uint processed = protocol.manualRolloverSeasons(projectID, toProcess);
        console.log("Processed:", processed);
        pending = protocol.getPendingSeasonRollovers(projectID);
    }
} else if (pending > 0) {
    // 50개 이하는 한 번에 처리
    protocol.manualRolloverSeasons(projectID, pending);
}

// 3. 정상화 확인
pending = protocol.getPendingSeasonRollovers(projectID);
console.log("Remaining:", pending);  // 0
...
contract StakingVault {
    ...
    function withdraw() external {
        ...
        (uint totalStaked,,) = stakingViewer.getStakeInfo(projectID, address(this));
...
contract ConditionalStaking {
    ...
    function autoStakeIfAPYAbove(uint minAPY) external payable {
        // 현재 APY 계산 (간단화)
        uint currentSeason = stakingViewer.getCurrentSeason(projectID);
        uint totalStaked = stakingViewer.getTotalStaked(projectID);
